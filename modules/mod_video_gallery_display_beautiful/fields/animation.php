...
*/
    ...
    class JFormFieldAnimation extends JFormField {
        ...
        public function makeInput($item, $name, $value=array())
        {
            ...
            $html='<div id="jhm-animation-setting-panel">';
                        ...
                        foreach($fieldset->field as $field)
                        {
                            ...
                            switch((string)$field['type'])
                            {
                                case 'text':
                                    ...
                                    name="'.$fieldname.'"
                                    style="'.$field['style'].'"
                                    value="'.$fieldvalue.'">  <div class="jhm-animation-unit">'.$field['unit'].'</div>  </li>';
                                    break;
        ...
        protected function getInput()
        {
            ...
            if( !isset($this->params['jhm_style'] ))
            {
                // JHM_SLIDER_DEFAULT  difined at fields/tmpl.php file :) 
                $this->params['jhm_style'] = JHM_SLIDER_DEFAULT;
            }



            $tmpl = JPATH_SITE.'/modules/mod_video_gallery_display_beautiful/tmpl';



            $defaultStyle = simplexml_load_file( $tmpl.'/'.$this->params['jhm_style'].'/config.xml' );
                    ...
                    foreach( $xml->files->admin->filename as $file)
                    {
                        if( isset($file['type']) and $file['type']=='javascript' )
                        {
                            if( isset($file['source']) and $file['source']=='external')
                            {
                                ...
                                $doc->addScript(    JURI::root(true).'/modules/mod_video_gallery_display_beautiful/tmpl/'.$folder.'/'.$file );
                            }
                        }  
                        ...
                        if( isset($file['type']) and $file['type']=='stylesheet' ){
                            ...
                            if( isset($file['source']) and $file['source']=='external')
                            {
                                ...
                                $doc->addStyleSheet( JURI::root(true).'/modules/mod_video_gallery_display_beautiful/tmpl/'.$folder.'/'.$file );
                            }
                        }
                    }
                }
            ...
            return $this->makeInput($defaultStyle->config, $this->params['jhm_style'], $this->params['animation']);
        }
}